...
void setxyLEDColor(const uint8_t x, const uint8_t y, const struct CRGB& color) {
	const uint16_t index = disco::xyIndexTable[x][y];
...
// mode 1 <- color fill based on disco::colorProfile
void discoModes::solidColorFill() {
	if(disco::colorProfile[0] == disco::colorProfile[MATRIX_HEIGHT - 1]) {
		fl::fill_solid(disco::leds, NUM_LEDS, disco::colorProfile[0]);
	} else {
		for (uint8_t y = 0,x; y < MATRIX_HEIGHT; ++y) {
			for (x = 0; x < MATRIX_WIDTH; ++x) {
				setxyLEDColor(x, y, disco::colorProfile[y]);
			}
		}
	}
}
...
void discoModes::audioReactiveVisualizer() {
	...
	for (int i = 0; i < NUM_LEDS; i++) {
		disco::leds[i].fadeToBlackBy(40);
	}

	for (int band = 0; band < MATRIX_WIDTH; band++) {
		int startBin = disco::logBins[band];
		int endBin = disco::logBins[band + 1];
		...
		for (int y = 1, index; y < height; y++) {
		  setxyLEDColor(flippedBand, MATRIX_HEIGHT - y, disco::colorProfile[y - 1]);
		}

		// Draw peak marker
		setxyLEDColor(flippedBand, MATRIX_HEIGHT - peakHeights[band], discoModes::peakColor);
	}
}
...
void discoModes::lowFrequencyPulseSync() {
	...
	fl::fill_solid(disco::leds, NUM_LEDS, color);
}


// mode 5 <- Ripple Beat
void discoModes::rippleBeat() {
	...
	fl::fill_solid(disco::leds, NUM_LEDS, CRGB::Black);
	if (rippleRadius >= 0) {
		for (int y = 0; y < MATRIX_HEIGHT; y++) {
			for (int x = 0; x < MATRIX_WIDTH; x++) {
				...
				if (dist == rippleRadius) {
					setxyLEDColor(x, y, CHSV((millis() / 5) % 255, 255, 255));
				}
			}	
		}
		rippleRadius++;
...
void discoModes::energySnake() {
	...
	double amp = 0;
	for (int i = 3; i < 10; i++) amp += disco::vReal[i];
	amp /= 7.0;
